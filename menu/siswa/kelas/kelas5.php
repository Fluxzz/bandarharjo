...
// Default kelas
$kelas = isset($_GET['kelas']) ? $_GET['kelas'] : '5A';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['nisn']) && isset($_POST['nama'])) {
        ...
        if (isset($_POST['nisnLama'])) {
            ...
            $updateQuery = "UPDATE siswa5 SET nama = ?, nisn = ?, kategori = ? WHERE nisn = ?";
            if ($stmt = $conn->prepare($updateQuery)) {
                $stmt->bind_param("ssss", $nama, $nisn, $kategori, $nisnLama);
                $stmt->execute();
                header("Location: kelas5.php?kelas=5$kategori");
                exit();
            }
        } else {
            // Proses Tambah
            $insertQuery = "INSERT INTO siswa5 (nama, nisn, kategori) VALUES (?, ?, ?)";
...
if (isset($_POST['hapus_nisn'])) {
    ...
    $deleteQuery = "DELETE FROM siswa5 WHERE nisn = ?";
    if ($stmt = $conn->prepare($deleteQuery)) {
        $stmt->bind_param("s", $nisnHapus);
        $stmt->execute();
        echo "<script>window.location.href = 'kelas5.php?kelas=5$kategori';</script>";
        exit();
    }
}

// Ambil data siswa
$query = "SELECT * FROM siswa5 WHERE kategori='$kategori'";
...
<div class="container">
    ...
    <div class="navbar-item">
        <div class="navigation">
            ...
            <div class="selection">
                <div class="option" onclick="window.location.href='?kelas=5A'"><p>Kelas 5A</p></div>
                <div class="option" onclick="window.location.href='?kelas=5B'"><p>Kelas 5B</p></div>
                <div class="option" onclick="window.location.href='?kelas=5C'"><p>Kelas 5C</p></div>
            </div>
        </div>
    </div>
